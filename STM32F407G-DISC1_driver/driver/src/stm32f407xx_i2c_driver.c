...
static void I2C_MasterHandleRXNEInterrupt(I2C_Handle_t *pI2CHandle)
{
	...
	if(pI2CHandle->RxLen == 0)
	{
		...
		I2C_ApplicationEventCallback(pI2CHandle, I2C_EV_RX_COMPLT);
...
void I2C_EV_IRQHandling(I2C_Handle_t *pI2CHandle)
{
		...
		if(pI2CHandle->TxRxState == I2C_BUSY_IN_TX)
		{
			if(pI2CHandle->pI2Cx->SR1 & (1<<I2C_SR1_TXE))
			{
				...
				if (pI2CHandle->TxLen == 0){
					...
					//3) notify application of completion of transmission event
					I2C_ApplicationEventCallback(pI2CHandle, I2C_EV_TX_COMPLT);
				}
			}
		}
	}
	...
	//Note: stop flag is applicable in slave mode. Master doesn't set it
	if(temp1 && temp3)
	{
		//STOPF flag is set
		//clear the STOPF (read SR1 and write to CR1)
		pI2CHandle->pI2Cx->CR1 |= 0x0000; //dummy write
		I2C_ApplicationEventCallback(pI2CHandle, I2C_EV_STOP); // notify the application that STOP is detected
	...
	{
		//check wether we are in master or slave mode
		if(pI2CHandle->pI2Cx->SR2 & (1<<I2C_SR2_MSL)){
		//TXE flag is set
			if(pI2CHandle->TxRxState == I2C_BUSY_IN_TX)
			{
